...
const LoginPage: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await AuthService.loginBusinessPanel(username, password);
      navigate('/business-panel/console-profiles');
    } catch (err) {
        ...
        <div className="bg-gray-800 rounded-lg shadow-lg p-8 border border-gray-700">
          <form onSubmit={handleSubmit}>
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full bg-cps-yellow text-black py-2 px-4 rounded-md hover:bg-yellow-500 focus:outline-none focus:ring-2 focus:ring-cps-yellow focus:ring-offset-2 focus:ring-offset-gray-800 transition-colors disabled:bg-gray-600 disabled:cursor-not-allowed"
            >
