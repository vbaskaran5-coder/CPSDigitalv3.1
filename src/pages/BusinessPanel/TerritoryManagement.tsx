...
const getAllBookingsFromSupabase = async (): Promise<MasterBooking[]> => {
  try {
    const seasonIds = REGIONS.flatMap(region =>
      region.seasons.map(season => season.id)
    );
    const bookings = await BookingService.getAllBySeasonIds(seasonIds);
    return bookings;
  } catch (err) {
