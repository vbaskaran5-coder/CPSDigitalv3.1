...
import { Worker, PayoutRecord, ConsoleProfile } from '../../types';
import { WorkerService, ConsoleProfileService } from '../../services/database.service';
...
const ContDetail: React.FC = () => {
  ...
  const handleSave = async () => {
    if (!workerId) return;
    try {
      await WorkerService.update(workerId, editForm);
      const updatedWorker = await WorkerService.getById(workerId);
  ...
  if (loading) {
    return (
      <div className="text-center text-gray-400 py-12">Loading worker details...</div>
    );
  }
