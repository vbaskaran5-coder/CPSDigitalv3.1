...
const MasterBookings: React.FC = () => {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      const bookingsFromStore = supabaseBookingStore.getAllBookings();
  ...
  const handleToggleExpand = (bookingId: string) => {
    ...
    if (expandedBookingId === bookingId) {
      ...
      const bookingToEdit = supabaseBookingStore.getBookingById(bookingId);
      if (bookingToEdit) {
  ...
  const handleSaveEdit = (
    ...
    try {
      ...
      supabaseBookingStore.updateBooking(bookingId, updates);
