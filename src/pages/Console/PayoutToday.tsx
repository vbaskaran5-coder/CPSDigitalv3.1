...
const PayoutToday: React.FC = () => {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        if (currentProfile.seasons && activeSeasonId) {
  ...
  const getBookingsForContractor = async (contractorId: string) => {
    const allWorkerBookings = await supabaseBookingStore.getBookingsForContractor(contractorId);
