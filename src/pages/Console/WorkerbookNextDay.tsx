...
const WorkerbookNextDay: React.FC = () => {
  ...
  const handleStatusClick = async (
    ...
    const worker = workers.find(w => w.contractorId === workerId);
    if (!worker) return;
    ...
    try {
      await WorkerService.update(workerId, updates);
    } catch (err) {
      console.error('Failed to update worker confirmation status:', err);
    }
  };
