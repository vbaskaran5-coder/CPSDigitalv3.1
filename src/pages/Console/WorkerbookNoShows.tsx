...
const WorkerbookNoShows: React.FC = () => {
  ...
  useEffect(() => {
    const loadWorkers = async () => {
      try {
        const allWorkers = await WorkerService.getByBookingStatus('no_show');
        const sorted = allWorkers.sort((a, b) => (b.noShows || 0) - (a.noShows || 0));
        setWorkers(sorted);
      } catch (err) {
        console.error('Failed to load no-show workers:', err);
      } finally {
        setLoading(false);
      }
    };
