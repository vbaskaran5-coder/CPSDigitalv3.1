...
const NewJob: React.FC = () => {
  ...
  useEffect(() => {
    const loadSeasonData = async () => {
      ...
      try {
        ...
        const consoleProfile = session?.profile as ConsoleProfile;

        if (!consoleProfile || !consoleProfile.id) {
          setError('Could not determine the correct console profile for settings.');
          return;
        }

        const seasonData = await ActiveSeasonService.get(consoleProfile.id);
        const activeSeasonHardcodedId = seasonData?.activeSeasonId;

        if (!activeSeasonHardcodedId) {
          setError('No active season is configured.');
          return;
        }
        ...
        setPayoutLogic(configuredSeason.payoutLogic || defaultPayoutLogicSettings);

        const today = new Date().toISOString().split('T')[0];
        const routeAssignments = await RouteAssignmentService.getByDate(today);
        const lastRouteCode = await AppStateService.getLastRouteCode(typeof loggedInWorkerId === 'string' ? loggedInWorkerId : String(loggedInWorkerId));
        ...
        setError('Failed to load season configuration.');
      }
    };
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const saveJob = async () => {
      try {
        await addJob(newBookingData);

        const session = await AuthService.getSession();
        const workerId = session?.userId;
        if (workerId) {
          await AppStateService.setLastRouteCode(
            typeof workerId === 'string' ? workerId : String(workerId),
            routeCode
          );
        }
